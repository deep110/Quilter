...
def matched_tiling(img, block_size, magnify_by = 2, overlap_size = None):
	...
	for i in range(0, n_blocks[0]):
		...
		for j in range(0, n_blocks[1]):
			...
			an = anchor.Anchor(output[row1:row2, col1:col2], row1, col1, overlap_size)
...
def matched_crop(img, block_size, anchor):
	...
	for i in range(0, max_size[0]):
		for j in range(0, max_size[1]):
			...
			curr_error = anchor.calcError(patch)
...
class Anchor(object):
	...
	def calcError(self, patch):
		...
		if(self.__col_index != 0):
			...
			total_error += self.__calcPixelError(b1, b2)
	...
	def __calcPixelError(self, block1, block2):
		diff = block1 - block2
		r,g,b = io.split_channels(diff*diff)
		r = 0.30*r
		g = 0.59*g
		b = 0.11*b
		e = np.sqrt(r+g+b)
