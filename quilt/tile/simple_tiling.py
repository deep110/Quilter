...
def simple_tiling(img, block_size, target_shape):

    n_blocks = (np.ceil(np.true_divide(target_shape[0:2], block_size))).astype('uint32')
    output = np.zeros(target_shape, 'uint8')

    for i in range(0, n_blocks[0]):
        row1 = i * block_size[0]
        row2 = min((i + 1) * block_size[0], target_shape[0])

        for j in range(0, n_blocks[1]):
            col1 = j * block_size[1]
            col2 = min((j + 1) * block_size[1], target_shape[1])
